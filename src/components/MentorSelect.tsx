    ...
    <div className="grid sm:grid-cols-2 gap-6 py-8 mt-10">
      {mentorProfiles.map((p) => {
        ...
        return (
          <Card
            key={p.key}
            className={`relative border-2 transition-all duration-200 hover:border-blue-400 cursor-pointer ${isSelected ? "border-blue-500 shadow-lg" : "border-transparent"
              }`}
            onClick={() => onSelect(p.key)}
          >
            <div
              className={`absolute top-2 right-2 bg-blue-500 rounded-full w-6 h-6 flex items-center justify-center transform transition-transform duration-300 ${isSelected ? "scale-100" : "scale-0"
                }`}
            >
              <svg
